...
example_medialab_guys <-function(){
  #done for Cesar, but not used
  #hidden used function
  ...
  for(dataset in datasets )
  {
    ...
    for(file in files)
    {
      ...
      pub <- process.publications(pub)
      pub['file'] <- file
      pub_total <- rbind(pub_total, pub, file)
  ...
  #backuping publications to work with
  write.csv(pub_total, file = "DATA_OUTPUT/publications_impact_2.csv", row.names = FALSE)
  
  
  #IF FILE ALREADY CREATED!
  pub_total <- read.csv(file = "DATA_OUTPUT/publications_impact_2.csv")
  ...
  #considering category according year which is based in j_c classification
  j_c["id_j_year"] <-paste(j_c$id_journal,j_c$id_year,sep="-")
  
  #merging publications, journals, categories
  pub_total["id_j_year"] <- paste(pub_total$journal_id, pub_total$Year, sep="-")
  
  #publications <- merge(pub_total, j_c, by.x = "journal_id", by.y = "id_journal")
  publications <- merge(pub_total, j_c, by.x  = "id_j_year", by.y="id_j_year")
  
  #adding names to categories
  publications <- merge(publications, nodes, by.x="id_category", by.y = "Id")
  
  write.csv(publications, file = "DATA_OUTPUT/publications_categories_unique.csv", row.names=FALSE)
  
  for(scholar in unique(publications$file)){
    library(d3plus) #name, group, value, color
    
    pub_s <- subset(publications, select = c("Title", "subd_name", "area_name", "Cites", "color" ), file==scholar)
    print(head(pub_s))
    pub_s['Cites'] <- as.numeric(as.character(pub_s$Cites))
    
    d3plus(type="tree", data= pub_s[,c("Title", "subd_name", "area_name", "Cites", "color") ], depth=1 )
    s<-d3plus(type="tree", data= pub_s[,c("Title", "subd_name", "area_name", "Cites", "color") ], depth=1 )
    htmlwidgets::saveWidget(s,paste("SCH",scholar, ".html", sep=""), selfcontained = FALSE)
  }
  
  
  
  #save HTML to external file
  s<-d3plus(type="tree", data= pub_s[,c("Title", "subd_name", "area_name", "Cites", "color") ], depth=0 )
  htmlwidgets::saveWidget(s,"categories_test.html", selfcontained = FALSE)
